...
**Goal:** 3 つのサービスページ末尾にインラインのフィードバックフォームを追加し、送信を Google Apps Script Web App プロキシ経由でサービスごとの Slack チャンネル + 共通 Google Spreadsheet に転送する仕組みを実装する。

**Architecture:** VitePress 静的サイトのクライアントが GAS Web App (匿名アクセス可) に `text/plain` で JSON POST し、GAS が (1) リクエスト検証、(2) Spreadsheet に行追記、(3) Slack Incoming Webhook へ Block Kit メッセージ転送、(4) Spreadsheet の `slack_status` 列を更新する。シークレットは GAS Script Properties に保持しクライアントには Web App URL と shared token のみ露出。
...
**Design Spec:** `docs/plans/2026-05-22-service-feedback-slack-design.md`
...
- [ ] Slack ワークスペースで Incoming Webhook を作成できる管理者権限
- [ ] Google アカウント (Spreadsheet と Apps Script プロジェクトを作成できる)
- [ ] GitHub リポジトリの Settings > Secrets and variables への書き込み権限
- [ ] ローカル開発環境 (`yarn` / Node 20 / VitePress 1.6.x が動作する)
...
## Task 1: Slack チャンネルと Incoming Webhook を 3 系統作成 (外部作業)

GAS のコードでは Webhook URL の値しか使わないため、コードは触らない準備タスク。

**Files:**
- Modify: なし (Slack 側設定のみ)

- [ ] **Step 1: Slack に 3 チャンネル作成**

Slack の左サイドバーから「+ チャンネルを追加」で以下を作成 (Private / Public は社内方針に従う):

- `#feedback-booking`
- `#feedback-soan`
- `#feedback-tsuzuri`

- [ ] **Step 2: 各チャンネルに Incoming Webhook を設定**

各チャンネルで以下を実施:

1. Slack アプリディレクトリで「Incoming Webhooks」アプリを検索し追加
2. 「Add to Slack」→ 投稿先チャンネルを選択 → 「Add Incoming WebHooks integration」
3. 表示された Webhook URL (`https://hooks.slack.com/services/T.../B.../...`) を控える

3 つの URL を取得したら次に進む。

- [ ] **Step 3: URL を一時的に安全な場所に保存**

3 つの URL を一時メモ (1Password 等) に保存。Task 3 で GAS Script Properties に登録するまでの一時保管。
...
- [ ] **Step 2: 既定の「シート 1」を `booking` にリネームしヘッダ行を書く**
...
- [ ] **Step 3: タブ `soan` と `tsuzuri` を追加**

下部の「+」で 2 タブ追加し、それぞれヘッダ行をコピー貼り付け。

- [ ] **Step 4: Spreadsheet ID を控える**
...
- [ ] **Step 1: `gas/feedback/appsscript.json` を作成**
...
- [ ] **Step 2: `gas/feedback/config.gs` を作成**
...
function getSlackWebhookUrl(service) {
  if (!ALLOWED_SERVICES.includes(service)) return null;
  const key = `SLACK_WEBHOOK_${service.toUpperCase()}`;
  return PROP.getProperty(key);
}

const SERVICE_META = {
  booking: { emoji: '📅', label: 'Calendar & Booking' },
  soan: { emoji: '📝', label: 'SOAN' },
  tsuzuri: { emoji: '✍️', label: 'Tsuzuri' },
};
```

- [ ] **Step 3: `gas/README.md` を作成**
...
`docs/plans/2026-05-22-service-feedback-slack-impl.md` の Task 11 を参照。
```
...
- [ ] **Step 1: `gas/feedback/tests.gs` にリクエスト検証テストを書く**
...
- [ ] **Step 2: テスト実行 (Apps Script UI で `runTests` を実行)**

Apps Script エディタで `runTests` 関数を選択 → 実行ボタン。`doPost` が未定義のため全件 FAIL することを確認。

- [ ] **Step 3: `gas/feedback/Code.gs` に doPost と検証ロジックを実装**
...
- [ ] **Step 4: テスト再実行で全件 PASS を確認**

Apps Script UI で `runTests` 実行 → ログに `all passed` 相当が出ることを確認。

- [ ] **Step 5: ローカル repo のソースも更新してコミット**
...
- [ ] **Step 1: テストを追加** (`gas/feedback/tests.gs`)
...
- [ ] **Step 3: `gas/feedback/spreadsheet.gs` を実装**
...
- [ ] **Step 5: `Code.gs` を `appendPendingRow` を呼ぶよう更新**
...
function test_slack_payload_shape() {
  ...
  if (!payload.blocks || payload.blocks.length !== 3) throw new Error('blocks shape');
  if (payload.blocks[0].type !== 'header') throw new Error('header missing');
  if (!payload.blocks[0].text.text.includes('SOAN')) throw new Error('service label missing');
  if (payload.blocks[1].text.text.indexOf('> テストメッセージ') !== 0) throw new Error('quote missing');
}
...
- [ ] **Step 3: `gas/feedback/slack.gs` を実装**
...
function postToSlack(service, message, timestamp, options) {
  ...
  const url = getSlackWebhookUrl(service);
...
- [ ] **Step 5: `Code.gs` に Slack 呼び出しと status 更新を組み込む**
...
- [ ] **Step 2: Spreadsheet にバインドした Apps Script プロジェクトを開く**

Spreadsheet を開いた状態で「拡張機能 > Apps Script」をクリック。

- [ ] **Step 3: `gas/feedback/` の全ファイルを Apps Script UI に貼る**
...
- [ ] **Step 4: Script Properties に 5 件登録**

「⚙ プロジェクト設定」→「スクリプト プロパティ」→「スクリプト プロパティを追加」:

- `SHARED_TOKEN` = Step 1 で生成した値
- `SPREADSHEET_ID` = Task 2 で控えた ID
- `SLACK_WEBHOOK_BOOKING` = Task 1 の URL
- `SLACK_WEBHOOK_SOAN` = Task 1 の URL
- `SLACK_WEBHOOK_TSUZURI` = Task 1 の URL

「スクリプト プロパティを保存」を押す。

- [ ] **Step 5: `runTests` で全テスト PASS を再確認**
...
期待: `{"ok":true}` が返り、Spreadsheet `soan` タブに 1 行追加、`#feedback-soan` に通知が届く。3 サービス全てで実施。
...
- [ ] **Step 1: `docs/.vitepress/theme/index.ts` を作成**
...
- [ ] **Step 3: コミットは Task 9 完了後にまとめて行う**
...
- [ ] **Step 1: `docs/services/booking.md` の末尾 (「ご利用にあたって」直前) にフィードバックセクションを追加**
...
- [ ] **Step 2: `docs/services/soan.md` の既存「フィードバック」セクションを置換**
...
- [ ] **Step 3: `docs/services/tsuzuri.md` の既存「フィードバック」セクションを置換**
...
- [ ] **Step 2: `.gitignore` を確認し `.env.local` が除外されているか確認**
...
- [ ] **Step 1: GitHub リポジトリの Settings > Secrets and variables > Actions で 2 件のシークレットを追加**
...
- [ ] **Step 2: `.github/workflows/deploy.yml` の「Build with VitePress」ステップに env を追加**
...
- [ ] **Step 2: Booking ページのフォームから疎通テスト**

「フィードバック」セクションの textarea に `ローカル疎通テスト (booking)` と入力 → 「送信する」 → 「お送りいただきありがとうございました。」が表示されることを確認。

- [ ] **Step 3: Slack と Spreadsheet を確認**

`#feedback-booking` チャンネルに通知が届き、Spreadsheet `booking` タブに 1 行追加されていることを目視確認。

- [ ] **Step 4: SOAN ページで同様に確認**

`http://localhost:5173/services/soan` → `ローカル疎通テスト (soan)` → 確認。

- [ ] **Step 5: Tsuzuri ページで同様に確認**

`http://localhost:5173/services/tsuzuri` → `ローカル疎通テスト (tsuzuri)` → 確認。

- [ ] **Step 6: Honeypot 動作確認 (devtools で属性差し替え)**
...
- [ ] **Step 8: エラー UX 動作確認 (一時的に GAS URL を壊す)**
...
- [ ] **Step 9: テスト用に挿入された Spreadsheet 行を削除**
...
- [ ] **Step 5: 本番テスト行を Spreadsheet から削除**
...
- **日次**: Slack `#feedback-*` チャンネルの未読を確認、対応必要なら Spreadsheet `notes` 列に記録
...
1. Slack ワークスペース管理画面で該当 Incoming Webhook を Revoke
2. 同じチャンネルに新しい Incoming Webhook を発行
3. Apps Script UI の「⚙ プロジェクト設定 > スクリプト プロパティ」で該当 `SLACK_WEBHOOK_<SERVICE>` を新しい URL に更新
...
例: 新サービス `newsvc` を追加する場合

1. Slack: `#feedback-newsvc` チャンネル作成 + Incoming Webhook
2. Spreadsheet: タブ `newsvc` を追加、ヘッダ行をコピー
3. Apps Script: Script Properties に `SLACK_WEBHOOK_NEWSVC` を追加
4. `gas/feedback/config.gs`:
   - `ALLOWED_SERVICES` に `'newsvc'` を追加
   - `SERVICE_META` に `newsvc: { emoji: '🆕', label: 'New Service' }` を追加
5. Apps Script UI の `config.gs` も同様に更新
6. `docs/services/newsvc.md` に `<ServiceFeedbackForm service="newsvc" />` を埋め込む
7. ローカル `yarn docs:dev` で疎通確認
8. コミット & push
...
2. **中期**: Cloudflare Turnstile を導入する別 plan を起こし、設計を更新する。`docs/plans/2026-05-22-service-feedback-slack-design.md` § 8 の「将来の CAPTCHA 追加」を参照
...
- [ ] **Step 3: push (Task 14 を先に済ませている場合は不要)**
